...
fn print_grove(elves: &Vec<(isize, isize)>) {
...
fn is_alone(elves: &Vec<(isize, isize)>, elf: &(isize, isize)) -> bool {
    !elves
        .iter()
        .any(|e| e != elf && (e.0 - elf.0).abs() <= 1 && (e.1 - elf.1).abs() <= 1)
}

fn move_elves(elves: &mut Vec<(isize, isize)>) {
        ...
        for elf in elves.iter() {
            ...
            for j in 0..4 {
                ...
                if can_propose_direction(elves, &elf, &direction) {
                    ...
                    elf_proposals.insert(elf.clone(), pos);
